from flask import Flask, request, jsonify, session, send_from_directory, redirect, url_for, flash, render_template
import os
import re
import sqlite3
...
class Database:
    ...
    def update_password(self, username, new_password):
        hashed_password = hashlib.sha256(new_password.encode()).hexdigest()  # Hashing the new password
        with self.connection:
            self.connection.execute('''
...
def send_image_to_gemini_api(image_path):
    """Send the uploaded image to the Gemini API and retrieve generated content."""
...
def update_age():
    if request.method == 'POST':
        ...
        if username:
            user = db.get_user(username)  # Get the user from the database
            if user:
                # Update the age in the ChatHistory table
                chat_history = ChatHistory.query.filter_by(username=username).first()
                if chat_history:
                    chat_history.age = age  # Set the new age
                    db.session.commit()  # Commit the changes
                    return jsonify({'message': 'Age updated successfully.'}), 200
                else:
                    return jsonify({'error': 'Chat history not found for the user.'}), 404
            else:
                return jsonify({'error': 'User not found.'}), 404
        else:
            return jsonify({'error': 'User is not logged in.'}), 401  # If user is not logged in
...
def upload_file():
    ...
    api_response = send_image_to_gemini_api(file_path)

    return jsonify({'message': 'File uploaded successfully', 'api_response': api_response}), 201
